<section class="section-newsletter">
    <div class="container">
        <div class="newsletter">
            <h1 class="newsletter__header">
                Subscribe to our newsletter
            </h1>
            <p class="newsletter__paragraph">
                Cras pulvinar mattis nunc sed blandit.
            </p>
            <form action="#" class="form">
                <div class="form__group">
                    <input type="email" class="form__input" required placeholder="Enter email">
                    <button type="submit" class="form__btn">
                        <span class="form__text">
                            Subscribe
                        </span>
                        <span class="form__icon">
                            <svg width="9" height="9" viewBox="0 0 9 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M8.79105 4.85355C8.98632 4.65829 8.98632 4.34171 8.79105 4.14645L5.60907 0.964466C5.41381 0.769204 5.09723 0.769204 4.90197 0.964466C4.7067 1.15973 4.7067 1.47631 4.90197 1.67157L7.73039 4.5L4.90197 7.32843C4.7067 7.52369 4.7067 7.84027 4.90197 8.03553C5.09723 8.2308 5.41381 8.2308 5.60907 8.03553L8.79105 4.85355ZM0.5625 5H8.4375V4H0.5625V5Z" fill="#F61067"/>
                            </svg>
                        </span>
                    </button>
                </div>
            </form>
            <form action="#" class="mobile-form">
                <div class="mobile-form__group">
                    <input type="text" class="mobile-form__input" required placeholder="Enter email">
                    <button type="submit" class="mobile-form__btn">
                        <svg width="9" height="9" viewBox="0 0 9 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M8.79105 4.85355C8.98632 4.65829 8.98632 4.34171 8.79105 4.14645L5.60907 0.964466C5.41381 0.769204 5.09723 0.769204 4.90197 0.964466C4.7067 1.15973 4.7067 1.47631 4.90197 1.67157L7.73039 4.5L4.90197 7.32843C4.7067 7.52369 4.7067 7.84027 4.90197 8.03553C5.09723 8.2308 5.41381 8.2308 5.60907 8.03553L8.79105 4.85355ZM0.5625 5H8.4375V4H0.5625V5Z" fill="#F61067"/>
                        </svg>
                    </button>
                </div>
            </form>
        </div>
    </div>
</section>